...
i_start, i_end = groups[args.group]
...
model_name = f"lstm_{group}_" + "_".join(train_trajs)
    ...
    for traj in trajs:
        for run in [1, 2, 3, 4, 5]:
            ...
            try:
                ...
                ds = QuadMultiStepDataset(df, horizon=horizon)
                datasets.append(ds)
...
train_ds = load_split(train_trajs, "../../data/real/processed/new/train")
valid_ds = load_split(valid_trajs, "../../data/real/processed/new/test")
...
# === Model Setup ===
# ---------------------------------------------------------------------
model = QuadLSTMModular(hidden_dim=64, state_dim_x=3, num_layers=2).to(device)
