...
from idsia_mpc.control_engineering_practice.sysid.models import PhysQuadModel, NeuralQuadModel, QuadMultiStepModel

# === CONFIG ===
setup_matplotlib()
mode = "residual"   # choose: "physics", "neural", "residual"
...
# quat_pred = X_torch_onestep[:, 6:10]
# euler_pred = quat_to_euler(quat_pred)
# X_torch_euler = np.hstack([X_torch_onestep[:, :6], euler_pred, X_torch_onestep[:, 10:]])  # shape (N-1, 12)
# Each rollout[i][0] corresponds to the one-step prediction from sample i
