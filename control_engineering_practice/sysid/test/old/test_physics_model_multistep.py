...
from control_engineering_practice.plot_utils import setup_matplotlib
...
phys_model = PhysQuadModel(params, dt).to(device)
...
X_first_from_rollouts = np.array([roll[0] for roll in X_torch_multistep])
...
# =====================================================
out_dir = f"../out/predictions/{type}/physics_model_multistep"
...
for i, roll in enumerate(X_torch_multistep):
    # skip empty or invalid rollouts
    if roll is None or len(roll) == 0 or not np.isfinite(roll).all():
        continue

    h = min(len(roll), max_h)
    roll = np.asarray(roll, dtype=np.float64)

    # normalize quaternions to avoid NaN in conversion
    quat_seq = roll[:h, 6:10]

    # convert quaternion -> euler
    euler_seq = quat_to_euler(quat_seq)
    if euler_seq.ndim == 1:
        euler_seq = euler_seq[np.newaxis, :]  # ensure shape (1,3)

    # assemble rollout in Euler format
    roll_euler = np.hstack([
        roll[:h, :6],       # pos + vel
        euler_seq,          # roll, pitch, yaw
        roll[:h, 10:]       # angular rates
    ])
