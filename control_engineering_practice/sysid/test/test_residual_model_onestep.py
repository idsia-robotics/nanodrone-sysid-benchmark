...
from idsia_mpc.control_engineering_practice.sysid.models import NeuralQuadModel, PhysQuadModel, ResidualQuadModel
...
data_type = "real"
traj_id_train = '1848'
traj_type_train = "random"
traj_id_test = '1809'
traj_type_test = "melon"
...
# === Load model ===
try:
    model_path = f"../out/models/residual_quad_model_onestep_{data_type}_{traj_type_train}_{traj_id_train}.pt"
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    raise RuntimeError(f"❌ Failed to load model weights from {model_path}.\n"
                       f"Error: {e}\n"
                       f"➡️ The saved checkpoint may not match your model architecture.")
model.eval()
...
out_dir = f"../out/predictions/{data_type}/residual_model_onestep"
