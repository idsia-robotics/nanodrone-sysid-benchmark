...
def plot_positions(t_vec, X_ref, X_m):
    """Plot position tracking x,y,z."""
    fig, axs = plt.subplots(3, 1, figsize=(8, 8), sharex=True)
    labels = ["x [m]", "y [m]", "z [m]"]
    for i, ax in enumerate(axs):
        ax.plot(t_vec, X_ref[:, i], "k--", label="ref")
        ax.plot(t_vec, X_m[:len(t_vec), i], "b", label="Mellinger")
...
def plot_velocities(t_vec, V_ref, X_m):
    """Plot velocity tracking vx,vy,vz."""
    fig, axs = plt.subplots(3, 1, figsize=(8, 8), sharex=True)
    labels = ["vx [m/s]", "vy [m/s]", "vz [m/s]"]
    for i, ax in enumerate(axs):
        ax.plot(t_vec, V_ref[:, i], "k--", label="ref")
        ax.plot(t_vec, X_m[:len(t_vec), 3+i], "b", label="Mellinger")
        ax.set_ylabel(labels[i]); ax.grid(True)
    axs[-1].set_xlabel("Time [s]"); axs[0].legend()
...
def plot_angular_rates(t_vec, W_ref, X_m):
    """Plot angular rates wx,wy,wz."""
    fig, axs = plt.subplots(3, 1, figsize=(8, 8), sharex=True)
    labels = ["wx [rad/s]", "wy [rad/s]", "wz [rad/s]"]
    for i, ax in enumerate(axs):
        ax.plot(t_vec, W_ref[:, i], "k--", label="ref")
        ax.plot(t_vec, X_m[:len(t_vec), 10+i], "b", label="Mellinger")
        ax.set_ylabel(labels[i]); ax.grid(True)
    axs[-1].set_xlabel("Time [s]"); axs[0].legend()
...
def plot_euler_angles(t_vec, Euler_ref, X_m):
    """Plot Euler angles roll, pitch, yaw."""
    Euler_m = np.array([X_m[i, 6:10] for i in range(len(t_vec))])  # quat
    # You’ll call quat_to_euler outside and pass arrays if preferred
    fig, axs = plt.subplots(3, 1, figsize=(8, 8), sharex=True)
    labels = ["roll [rad]", "pitch [rad]", "yaw [rad]"]
    for i, ax in enumerate(axs):
        ax.plot(t_vec, Euler_ref[:, i], "k--", label="ref")
        ax.plot(t_vec, Euler_m[:, i], "b", label="Mellinger")
        ax.set_ylabel(labels[i]); ax.grid(True)
    axs[-1].set_xlabel("Time [s]"); axs[0].legend()
